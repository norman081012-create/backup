...
def ability_slider(label, key, current_val, wealth, dept_cap, cfg):
    ...
    if invest < maint:
        st.caption(f"📉 投入不足維持，降至: {next_val*10:.1f}% | 升級花費: {invest} | 明年維護: {int(next_maint)}")
    else:
        st.caption(f"📈 升級至: {min(100.0, next_val*10):.1f}% | 升級花費: {invest} | 明年維護: {int(next_maint)}")
    return invest
