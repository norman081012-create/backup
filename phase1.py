...
def render(game, view_party, cfg):
    ...
    if game.p1_step in ['draft_r', 'draft_h', 'ultimatum_draft_r']:
        ...
        if view_party.name != (game.r_role_party.name if active_role == 'R' else game.h_role_party.name):
            ...
            st.markdown(f"#### 📝 {view_party.name} ({role_text_zh}黨) 草案擬定室")
            ...
            st.markdown(f"**公告衰退值 (當前智庫估算: {st.session_state[input_key]:.2f})** | {opp_txt}")
            ...
            proj_fund = st.slider("標案總額 (最高不超過當年總預算)", 0.0, max_p, float(min(1000.0, max_p)), 10.0)
            bid_cost = st.slider("標案成本 (要求之建設產出值，留點利潤給對手賺)", 1.0, max(1.0, float(proj_fund)), max(1.0, float(proj_fund * 0.8)), 10.0)
            
            safe_req = max(1.0, float(proj_fund))
            r_pays = st.slider("💰 資金分配調整 (監管出資額)", 0.0, safe_req, float(safe_req * 0.5))
            h_pays = proj_fund - r_pays
            r_pct = (r_pays / max(1.0, proj_fund)) * 100
            h_pct = (h_pays / max(1.0, proj_fund)) * 100
            
            st.markdown(f"<h4><span style='font-size: 1.2em'>監管出資: {r_pays:.1f} ({r_pct:.1f}%)</span> / 總額: {proj_fund:.1f} / <span style='font-size: 1.2em'>執行出資: {h_pays:.1f} ({h_pct:.1f}%)</span></h4>", unsafe_allow_html=True)
            ...
            if active_role == 'R' and game.p1_step == 'draft_r':
                ...
                if c_btn3.button(f"🔄 強制通過並換位 (費用: {swap_cost:.1f})", use_container_width=True):
            ...
            st.markdown("---")
            ui_core.render_proposal_component("📜 當前草案預覽與智庫分析", plan_dict, game, view_party, cfg)
            if opp_plan:
                st.markdown("---")
        ...
        if view_party.name != game.proposing_party.name: st.warning("⏳ 等待對手覆議...")
            ...
            if c3.button(f"🔄 同意但換位\n(各付 {penalty_amt:.1f})", use_container_width=True):
                st.session_state.turn_data.update(game.p1_selected_plan)
        ...
        if view_party.name != game.h_role_party.name: 
            ...
            if c1.button("✅ 忍辱負重 (接受通牒)", use_container_width=True, type="primary"):
                st.session_state.turn_data.update(game.p1_selected_plan)
                st.session_state.news_flash = "🗞️ **【快訊】通牒生效！** 執行系統妥協吞下底線方案。"
                st.session_state.anim = 'balloons'
                game.phase = 2; game.proposing_party = game.ruling_party; st.rerun()
            if c2.button(f"🔄 掀桌倒閣換位\n(警告: 各付 {penalty_amt:.1f})", use_container_width=True):
                st.session_state.turn_data.update(game.p1_selected_plan)
