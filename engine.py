...
def render_dashboard(game, view_party, opponent, cfg, is_preview=False, preview_data=None):
    ...
    with c1:
        st.markdown("### 🕵️ 情報處 (資訊預估)")
        fc = view_party.current_forecast
        think_acc = min(100, int((view_party.predict_ability / cfg['MAX_ABILITY']) * 100))
        st.info(f"經濟衰退預估: -{fc:.2f}\n\n智庫準確度: {think_acc}% ({config.get_accuracy_text(think_acc)})")
        
        # 情報處準確度 = 己方情報能力 - 敵方反情報(隱密)能力
        intel_acc = max(0, min(100, int((view_party.investigate_ability - opponent.covert_ability) / cfg['MAX_ABILITY'] * 100)))
        st.warning(f"對手貪污掌握度: {intel_acc}% ({config.get_accuracy_text(intel_acc)})")

    with c2:
        st.markdown("### 📊 審計處 (資源與目標)")
    ...
    with c4:
        if is_preview:
            ...
            if rep:
                ...
                st.write(f"**去年智庫表現:** {config.get_thinktank_eval(view_party.predict_ability, diff)}")
                st.markdown(f"**淨利估算誤差:** 真:{r_inc:.0f} (估:{e_inc:.0f})")
                
                poll = view_party.current_poll_result
                if poll is not None:
                    sup_shift = rep['h_sup_shift'] if my_is_h else rep['r_sup_shift']
                    st.markdown(f"**大選民調預測:** {poll:.1f} ({sup_shift:+.1f})")
                else: st.markdown("**大選民調預測:** ??? (需作民調)")
            else:
                st.info("第一年尚無歷史檢討資料。")
    st.markdown("---")
...
def render_message_board(game):
    ...
    if game.phase == 1:
        ...
        st.info("🛠️ **【策略建議】** 法案已通過，請分配黨產資金進行內部部門投資、競選造勢與媒體攻防。")
...
def render_strategic_intelligence_center(party, cfg, is_self=True, blur=0.0):
    total_maint = sum([get_maint_cost(a, cfg) for a in [party.build_ability, party.investigate_ability, party.covert_ability, party.media_ability, party.predict_ability]])
    total_cost = party.last_acts['policy'] - party.last_acts['legal'] + total_maint
    est_income = max(1, party.wealth)
    
    if is_self:
        st.markdown("##### 📊 內部審計處 (本年營運預估)")
        st.write(f"**可用淨資產:** `{party.wealth:.0f}` - `{total_cost:.0f}` = **{party.wealth - total_cost:.0f}**")
        with st.expander("詳細開銷佔比"):
...
def render_party_cards(game, view_party, opponent_party, god_mode, is_election_year, cfg):
    st.header("👤 玩家頁面")
    c1, c2 = st.columns(2)
    opp = opponent_party
    ...
    for col, party in zip([c1, c2], [view_party, opp]):
        with col:
            ...
            status_text = "當權!" if is_winner else "候選"
            
            if is_election_year or god_mode: 
                disp_sup = f"{party.support:.1f}% 🏆({status_text})"
            else:
                disp_sup = f"📊 {party.current_poll_result:.1f}%" if party.current_poll_result is not None else "??? (需作民調)"
            
            intel_acc = max(0.0, min(1.0, (view_party.investigate_ability - opp.covert_ability) / cfg['MAX_ABILITY']))
            blur = 1.0 - intel_acc if not god_mode else 0.0
...
def render_proposal_component(title, plan, game, view_party, cfg):
    ...
    with c_left:
        ...
        st.markdown(f"**總需求:** `{plan['total_funds']}` | 監管出資:`{plan['r_pays']}` | 執行出資:`{plan['h_pays']}`")
    
    with c_right:
        st.markdown("📊 **👁️ 智庫評估報告:**")
...
def ability_slider(label, key, current_val, wealth, dept_cap, cfg):
    maint = max(0, (current_val - 3.0) * cfg['MAINTENANCE_RATE'])
    max_invest = min(int(wealth), int(dept_cap))
    default_val = min(int(maint), max_invest)
    
    invest = st.slider(f"{label} (當前: {current_val*10:.1f}%)", 0, max_invest, default_val, key=key)
    ...
    st.caption(f"升級花費: {invest} | 明年維護: {int(next_maint)} | 預期能力: {min(100.0, next_val*10):.1f}%")
    return invest
