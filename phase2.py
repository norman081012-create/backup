...
def render(game, view_party, opponent_party, cfg):
    ...
    if tot <= current_wealth and st.button("確認行動/結算", use_container_width=True, type="primary"):
        ...
        if f"{opponent_party.name}_acts" not in st.session_state:
            ...
            game.sanity = max(0.0, min(1.0, game.sanity - (game.emotion * 0.002))) # 教育移除後自然衰減
            ...
            rp.covert_ability, _ = formulas.get_ability_preview(rp.covert_ability, ra['p_cov'], cfg)
            ...
            hp.covert_ability, _ = formulas.get_ability_preview(hp.covert_ability, ha['p_cov'], cfg)
