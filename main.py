...
if 'turn_initialized' not in st.session_state:
    ...
    if game.year == 1:
        st.session_state.news_flash = f"🎉 **[FOUNDING ELECTION]** Game Start! The {game.ruling_party.name} party has secured the first term!"
...
if god_mode:
    ...
    st.error(t(f"👁️ **GOD MODE:** True Decay is **{game.current_real_decay:.3f}** (Infra Loss: {real_loss:.1f})"))
